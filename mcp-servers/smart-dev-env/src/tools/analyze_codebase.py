...
async def _analyze_full(target_path: Path) -> Dict:
    ...
    if target_path.is_file():
        ...
        for ext in extensions:
            for file_path in target_path.rglob(f"*{ext}"):
                ...
                if not any(excluded in file_path.parts for excluded in exclude_patterns):
    ...
    analysis["summary"] = {
        ...
        "average_complexity": round(
            total_complexity / max(len(analysis["files"]), 1), 2
        ),
        "languages_detected": list(
            set(f.get("language", "unknown") for f in analysis["files"])
        ),
    }
    ...
    analysis["metrics"] = {
        "complexity_score": _calculate_complexity_score(
            total_complexity, len(analysis["files"])
        ),
...
async def _analyze_single_file(file_path: Path) -> Dict:
    ...
    try:
        ...
        file_analysis["complexity"] = _calculate_file_complexity(
...
def _find_file_issues(file_path: Path, lines: List[str]) -> List[Dict]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if (
            file_path.suffix in [".java", ".js", ".ts", ".cs"]
            and "catch" in line_stripped.lower()
        ):
...
def _calculate_quality_score(issues: List[Dict], total_lines: int) -> int:
    ...
    weighted_issues = sum(
        severity_weights.get(issue.get("severity", "low"), 1) for issue in issues
    )
...
def _calculate_technical_debt(issues: List[Dict], total_lines: int) -> Dict:
    ...
    return {
        ...
        "debt_ratio": round(
            (len(issues) / max(total_lines, 1)) * 1000, 2
...
def _generate_recommendations(
    metrics: Dict, issues: List[Dict], summary: Dict
) -> List[str]:
    ...
    if complexity_score < 60:
        ...
        recommendations.append(
            "📊 Monitor complexity - some functions are getting complex"
        )
    ...
    if effort_hours > 20:
        recommendations.append(
            f"⏰ High technical debt detected - {effort_hours} hours estimated to resolve"
        )
    elif effort_hours > 5:
        recommendations.append(
            f"📈 Consider dedicating time to technical debt ({effort_hours} hours estimated)"
        )
    ...
    if total_files > 100:
        recommendations.append(
            "📁 Large codebase - consider modularization and dependency management"
        )

    # Positive feedback
    if not recommendations or (
        complexity_score > 80 and quality_score > 80 and maintainability > 80
    ):
        recommendations.append(
            "✨ Code quality looks excellent! Consider adding automated quality gates"
        )
...
async def _analyze_complexity(target_path: Path) -> Dict:
    ...
    high_complexity_files = [
        f for f in full_analysis["files"] if f.get("complexity", 0) > 20
    ]
...
async def _analyze_quality(target_path: Path) -> Dict:
    ...
    return {
        ...
        "recommendations": [
            ...
            for rec in full_analysis["recommendations"]
            if any(
                word in rec.lower() for word in ["quality", "issue", "clean", "debug"]
            )
        ],
...
async def _analyze_security(target_path: Path) -> Dict:
    ...
    security_issues = [
        issue
        for issue in full_analysis["issues"]
        if issue.get("type") in ["hardcoded_secret", "empty_catch"]
    ...
    return {
        ...
        "security_metrics": {
            ...
            "risk_level": "high"
            if any(i.get("type") == "hardcoded_secret" for i in security_issues)
            else "medium"
            if security_issues
            else "low",
