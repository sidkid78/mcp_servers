...
async def rollback_changes_tool(
    target: str, identifier: str, confirm: bool = False
) -> Dict:
    """
    Safe rollback mechanisms for deployments or code changes.
    """
...
async def _perform_rollback(target: str, identifier: str) -> Dict:
    ...
    try:
        ...
        rollback_info["recovery_instructions"] = _generate_recovery_instructions(
            rollback_info
        )
...
async def _rollback_commit(commit_hash: str, rollback_info: Dict) -> Dict:
    ...
    try:
        ...
        rollback_info["steps"].append(
            {"step": "Create backup branch", "status": "in_progress"}
        )
        ...
        if backup_result.returncode == 0:
            ...
            rollback_info["steps"].append(
                {"step": "Reset to commit", "status": "in_progress"}
            )
        ...
        if reset_result.returncode == 0:
            ...
            rollback_info["message"] = (
                f"Successfully rolled back to commit {commit_hash}"
            )
        else:
            rollback_info["status"] = "failed"
            rollback_info["steps"][-1]["status"] = "failed"
...
async def _rollback_deployment(deployment_id: str, rollback_info: Dict) -> Dict:
    ...
    try:
        ...
        rollback_info["steps"].append(
            {"step": "Validate deployment ID", "status": "passed"}
        )

        # Step 2
        rollback_info["previous_deployment"] = (
            f"deploy-prev-{hash(deployment_id) % 10000}"
        )

        # Step 3: Switch traffic back
        rollback_info["steps"].append(
            {"step": "Switch traffic to previous version", "status": "success"}
        )
        ...
        rollback_info["status"] = "success"
        rollback_info["message"] = (
            f"Successfully rolled back deployment {deployment_id}"
        )
        rollback_info["rollback_url"] = "https://app.example.com/previous-version"
...
async def _rollback_migration(migration_version: str, rollback_info: Dict) -> Dict:
    ...
    try:
        ...
        rollback_info["steps"].append(
            {"step": "Validate migration version", "status": "passed"}
        )

        # Step 2: Create database backup
        rollback_info["steps"].append(
            {"step": "Create database backup", "status": "success"}
        )
        rollback_info["backup_created"] = True
        rollback_info["backup_location"] = (
            f"backup-before-migration-rollback-{migration_version}"
        )

        # Step 3: Run migration rollback
        rollback_info["steps"].append(
            {"step": "Execute migration rollback", "status": "success"}
        )

        # Step 4: Verify database state
        rollback_info["steps"].append(
            {"step": "Verify database state", "status": "success"}
        )

        rollback_info["status"] = "success"
        rollback_info["message"] = (
            f"Successfully rolled back migration {migration_version}"
        )
...
async def _rollback_release(release_tag: str, rollback_info: Dict) -> Dict:
    ...
    try:
        ...
        rollback_info["steps"].append(
            {"step": "Validate release tag", "status": "checking"}
        )
        ...
        rollback_info["steps"].append(
            {"step": "Find previous release", "status": "success"}
        )

        # Step 3: Rollback to previous release
        rollback_info["steps"].append(
            {"step": "Rollback to previous release", "status": "success"}
        )
    ...
    if status == "success":
        if target == "commit":
            ...
            if rollback_info.get("rollback_url"):
                instructions.append(
                    f"Previous version available at: {rollback_info['rollback_url']}"
                )
        ...
        if rollback_info.get("backup_created"):
            instructions.append(
                "Backup was created and is available for manual recovery"
            )
