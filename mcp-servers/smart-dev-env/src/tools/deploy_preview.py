...
async def _perform_deployment(environment: str, branch: str, notify: bool) -> Dict:
    ...
    try:
        ...
        deployment_info["steps"].append(
            {"step": "Build application", "status": build_result["status"]}
        )

        # Step 3: Deploy to environment
        deploy_result = await _deploy_to_environment(environment, branch)
        deployment_info["steps"].append(
            {"step": f"Deploy to {environment}", "status": deploy_result["status"]}
        )
        ...
        if all(
            step["status"] in ["passed", "success"] for step in deployment_info["steps"]
        ):
        ...
        # Send notifications
        if notify:
            deployment_info["notifications"] = await _send_notifications(
                deployment_info
            )
