...
async def check_dependencies_tool(
    ...
    try:
        ...
        analysis = await _analyze_dependencies(
            manifest_file, dep_type, check_vulnerabilities, check_updates
        )
...
async def _analyze_dependencies(
    ...
    try:
        if dep_type == "npm":
            analysis.update(
                await _check_npm_dependencies(manifest_file, check_vulnerabilities)
            )
        elif dep_type == "pip":
            analysis.update(
                await _check_pip_dependencies(manifest_file, check_vulnerabilities)
            )
        else:
...
async def _check_npm_dependencies(
    manifest_file: Path, check_vulnerabilities: bool
) -> Dict:
...
async def _check_pip_dependencies(
    manifest_file: Path, check_vulnerabilities: bool
) -> Dict:
        ...
        for line in lines:
            ...
            if line and not line.startswith("#"):
                if "==" in line:
                    ...
                    dependencies.append(
                        {"name": line, "version": "*", "type": "production"}
                    )
...
async def _check_generic_dependencies(manifest_file: Path, dep_type: str) -> Dict:
    ...
    return {
        ...
        "vulnerabilities": [],
        "summary": {
            "message": f"Generic analysis for {dep_type} - limited functionality"
        },
...
def _generate_dependency_recommendations(analysis: Dict) -> List[str]:
    ...
    if vulnerabilities:
        recommendations.append(
            f"Address {len(vulnerabilities)} security vulnerabilities"
        )
