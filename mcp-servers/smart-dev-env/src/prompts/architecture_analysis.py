...
async def architecture_analysis_prompt(
    component: str, focus: str = "maintainability"
) -> str:
    """
...
async def _detect_architectural_patterns() -> Dict:
    ...
    for pattern_name, indicators in pattern_indicators.items():
        ...
        if matches > 0:
            detected.append(
                {"pattern": pattern_name, "confidence": matches / len(indicators) * 100}
            )
...
async def _analyze_component(component: str, project_analysis: Dict) -> Dict:
    ...
    if component_path.exists():
        files = (
            [str(f) for f in component_path.rglob("*.*")][:10]
        ...
        return {
            "name": component,
            "details": f"Component with {len(files)} files"
            if component_path.is_dir()
            else "Single file component",
            "files": files,
...
async def _generate_architecture_recommendations(
    focused_analysis: Dict, focus: str
) -> Dict:
