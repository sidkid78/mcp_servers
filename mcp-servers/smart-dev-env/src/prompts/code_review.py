...
def _check_quality_issues(file_path: str, lines: List[str]) -> List[Dict]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if any(
            debug in line.lower() for debug in ["console.log", "print(", "debugger"]
        ):
            issues.append(
                {
                    "type": "debug_code",
...
async def _analyze_security(files: List[str]) -> Dict:
    ...
    if any(
        v["type"] in ["sql_injection", "xss", "hardcoded_secrets"]
...
async def _analyze_test_coverage(files: List[str]) -> Dict:
    ...
    for file_path in files:
        if any(
            pattern in file_path.lower() for pattern in ["test", "spec", "__test__"]
        ):
...
""")
    ...
    return (
        ...
        if report_sections
        else "Analysis complete - no major issues detected."
    )
...
def _format_recommendations(analysis_results: Dict, severity: str) -> str:
    ...
    if severity == "critical":
        recommendations.insert(
            0,
            "Critical review mode: Address all high-severity issues before deployment",
        )
