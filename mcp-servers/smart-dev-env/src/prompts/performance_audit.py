...
async def _define_audit_scope(scope: str) -> Dict:
    ...
    if "frontend" in scope_lower:
        audit_scope["areas"] = ["UI rendering", "JavaScript execution", "Asset loading", "Bundle size"]
        audit_scope["description"] = "Frontend performance analysis focusing on user experience"
    elif "backend" in scope_lower:
        audit_scope["areas"] = ["API response times", "Database queries", "Server resources", "Memory usage"]
        audit_scope["description"] = "Backend performance analysis focusing on server efficiency"
    elif "database" in scope_lower:
        audit_scope["areas"] = ["Query performance", "Index optimization", "Connection pooling"]
        audit_scope["description"] = "Database performance analysis focusing on data access"
    else:
        audit_scope["areas"] = ["General performance", "Resource usage", "Response times"]
...
def _format_bottlenecks(bottlenecks: List[Dict]) -> str:
    ...
    for bottleneck in bottlenecks:
        impact_emoji = "🔴" if bottleneck["impact"] == "high" else "🟡" if bottleneck["impact"] == "medium" else "🟢"
        formatted.append(f"{impact_emoji} **{bottleneck['type'].title()}:** {bottleneck['description']}")
...
def _format_optimization_plan(plan: Dict) -> str:
    ...
    impact_emoji = "🔴" if plan["estimated_impact"] == "high" else "🟡" if plan["estimated_impact"] == "medium" else "🟢"
    formatted.append(f"{impact_emoji} **Estimated Impact:** {plan['estimated_impact'].title()}")
    formatted.append("")
...
def _suggest_performance_next_steps(bottlenecks: List[Dict], optimization_plan: Dict) -> str:
