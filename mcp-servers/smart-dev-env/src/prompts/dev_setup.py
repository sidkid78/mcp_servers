...
async def _analyze_project_structure(project_path: Path) -> Dict:
    ...
    try:
        ...
        for root, dirs, files in os.walk(project_path):
            ...
            for file in files:
                if not file.startswith('.'):
                    ...
                    # Identify key files
                    if file in ['package.json', 'requirements.txt', 'Cargo.toml', 'go.mod', 'pom.xml', 'Dockerfile', 'docker-compose.yml']:
        ...
        structure["structure_summary"] = f"{size} project ({structure['total_files']} files, {len(structure['directories'])} directories)"
...
def _format_standards(standards: Dict) -> str:
    ...
    for category, files in standards.items():
        if files:
            formatted.append(f"{icons.get(category, '•')} {category.replace('_', ' ').title()}: {', '.join(files)}")
    
    return '\n'.join(formatted)


def _generate_quick_start_suggestions(project_info: Dict, tech_stack: Dict, standards: Dict) -> str:
    ...
    if project_info["total_files"] > 100:
        suggestions.append("📊 Large codebase detected → Try `/smart-dev/architecture-analysis` to understand structure")
    
    if not standards["testing"]:
        suggestions.append("🧪 No testing config found → Use `/smart-dev/refactor-planning` to establish testing")
    
    if "React" in tech_stack["technologies"] or "Vue.js" in tech_stack["technologies"]:
        suggestions.append("⚡ Frontend project → Consider `/smart-dev/performance-audit frontend` for optimization")
    
    if "Docker" in tech_stack["technologies"]:
        suggestions.append("🐳 Docker detected → Use `deploy-preview` tool for containerized deployments")
    
    if not suggestions:
        suggestions.append("🔍 Run `/smart-dev/code-review HEAD` to review recent changes")
        suggestions.append("🏛️ Try `/smart-dev/architecture-analysis` to explore system design")
